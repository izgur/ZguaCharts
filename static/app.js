...
function setupLearningControls() {
  ...
  document.querySelector("#research-paper-candidates-refresh")?.addEventListener("click", loadResearchPaperCandidates);
...
function formatAutopilotGate(gate) {
  ...
  return [gate.name, gate.detail].filter(Boolean).join(": ") || "-";
}
...
function renderAutopilotJob(job) {
  ...
  const evidence = job.previousEvidenceSummary || {};
  const gates = (evidence.failedGates || []).slice(0, 5).map((gate) => `<li>${escapeHtml(formatAutopilotGate(gate))}</li>`).join("");
  ...
  return `
    <details class="paper-validation-result">
      <summary><strong>${escapeHtml(job.status || "QUEUED")}</strong> ${escapeHtml(strategies.join(", ") || "-")} ${escapeHtml(symbols.join(", ") || "-")} ${escapeHtml(timeframes.join(", ") || "-")} <span class="neutral">${escapeHtml(sourceLabel)}</span></summary>
      <div class="metric-grid">
        <div class="metric"><span>Job ID</span><strong>${escapeHtml(job.jobId || "-")}</strong></div>
        <div class="metric"><span>Status</span><strong>${escapeHtml(job.status || "QUEUED")}</strong></div>
        <div class="metric"><span>Priority</span><strong>${job.priority ?? "-"}</strong></div>
        <div class="metric"><span>Generated by</span><strong>${escapeHtml(job.generatedBy || "-")}</strong></div>
        <div class="metric"><span>Strategies</span><strong>${escapeHtml(strategies.join(", ") || "-")}</strong></div>
        <div class="metric"><span>Symbols</span><strong>${escapeHtml(symbols.join(", ") || "-")}</strong></div>
        <div class="metric"><span>Timeframes</span><strong>${escapeHtml(timeframes.join(", ") || "-")}</strong></div>
        <div class="metric"><span>Period</span><strong>${escapeHtml(job.period || "-")}</strong></div>
        <div class="metric"><span>Max combos</span><strong>${job.maxCombosPerStrategy ?? "-"}</strong></div>
        <div class="metric"><span>Top N</span><strong>${job.topN ?? "-"}</strong></div>
        <div class="metric"><span>Stress</span><strong>${job.includeStress ? "yes" : "no"}</strong></div>
        <div class="metric"><span>Recent</span><strong>${job.includeRecentWindows ? "yes" : "no"}</strong></div>
        <div class="metric"><span>Repro</span><strong>${job.includeReproAudit ? "yes" : "no"}</strong></div>
        <div class="metric"><span>Timeout</span><strong>${job.timeoutSeconds ?? "-"}</strong></div>
        <div class="metric"><span>Parent</span><strong>${escapeHtml(job.parentJobId || "-")}</strong></div>
        <div class="metric"><span>Created</span><strong>${escapeHtml(job.createdAt || "-")}</strong></div>
        <div class="metric"><span>Started</span><strong>${escapeHtml(job.startedAt || "-")}</strong></div>
        <div class="metric"><span>Finished</span><strong>${escapeHtml(job.finishedAt || "-")}</strong></div>
        <div class="metric"><span>Saved</span><strong>${escapeHtml(job.savedPath || result.savedPath || "-")}</strong></div>
        <div class="metric"><span>Result code</span><strong>${job.resultStatusCode ?? "-"}</strong></div>
      </div>
      <p class="modal-note"><strong>Reason:</strong> ${escapeHtml(job.reason || "-")}</p>
      <p class="modal-note"><strong>Result:</strong> ${escapeHtml(result.recommendation?.action || result.recommendation?.reason || job.error || "-")}</p>
      <h4 class="modal-section-title">Why This Job Exists</h4>
      <div class="metric-grid">
        <div class="metric"><span>Branch</span><strong>${escapeHtml(evidence.branchKey || "-")}</strong></div>
        <div class="metric"><span>Strategy</span><strong>${escapeHtml(evidence.strategy || "-")}</strong></div>
        <div class="metric"><span>Symbol</span><strong>${escapeHtml(evidence.symbol || "-")}</strong></div>
        <div class="metric"><span>Timeframe</span><strong>${escapeHtml(evidence.timeframe || "-")}</strong></div>
        <div class="metric"><span>Period</span><strong>${escapeHtml(evidence.period || "-")}</strong></div>
        <div class="metric"><span>PF</span><strong>${formatNumber(evidence.profitFactor)}</strong></div>
        <div class="metric"><span>Return</span><strong>${formatSigned(evidence.totalReturnPct || 0)}%</strong></div>
        <div class="metric"><span>Trades</span><strong>${evidence.fullTrades ?? "-"}</strong></div>
        <div class="metric"><span>Category</span><strong>${escapeHtml(evidence.reasonCategory || "-")}</strong></div>
        <div class="metric"><span>Source</span><strong>${escapeHtml(evidence.sourceReport || "-")}</strong></div>
      </div>
      ${gates ? `<ul class="backtest-warnings">${gates}</ul>` : `<p class="modal-note">No parent failed gates recorded.</p>`}
    </details>
  `;
}
...
function renderAutopilotFamily(family) {
  const best = family.bestBranch || {};
  return `
    <details class="paper-validation-result">
      <summary><strong>${escapeHtml(family.strategy || "-")}</strong> <span class="neutral">${escapeHtml(family.familyStatus || "ACTIVE")}</span></summary>
      <p class="modal-note">${escapeHtml(family.reason || "-")}</p>
      <div class="metric-grid">
        <div class="metric"><span>Branches</span><strong>${family.branchesTested ?? 0}</strong></div>
        <div class="metric"><span>Rejected</span><strong>${family.rejectedBranches ?? 0}</strong></div>
        <div class="metric"><span>Negative</span><strong>${family.negativeReturnBranches ?? 0}</strong></div>
        <div class="metric"><span>Stress collapse</span><strong>${family.stressCollapseBranches ?? 0}</strong></div>
        <div class="metric"><span>Weak recent</span><strong>${family.recentlyWeakBranches ?? 0}</strong></div>
        <div class="metric"><span>Insufficient</span><strong>${family.insufficientEvidenceBranches ?? 0}</strong></div>
        <div class="metric"><span>Promising rare</span><strong>${family.promisingRareBranches ?? 0}</strong></div>
        <div class="metric"><span>Eligible/stable</span><strong>${family.eligibleOrStableBranches ?? 0}</strong></div>
        <div class="metric"><span>Best branch</span><strong>${escapeHtml(best.branchKey || "-")}</strong></div>
        <div class="metric"><span>Best PF</span><strong>${formatNumber(family.bestPF)}</strong></div>
        <div class="metric"><span>Best return</span><strong>${formatSigned(family.bestReturnPct || 0)}%</strong></div>
        <div class="metric"><span>Best trades</span><strong>${family.bestTrades ?? "-"}</strong></div>
      </div>
      <p class="modal-note"><strong>Next action:</strong> ${escapeHtml(family.recommendedNextAction || "-")}</p>
    </details>
  `;
}
...
function renderResearchAutopilot(payload, action = "status") {
  if (action === "journal") {
    ...
    const families = (payload.strategyFamilies || []).map(renderAutopilotFamily).join("");
    return `
      ...
      <h4 class="modal-section-title">Strategy Families</h4>
      ${families || `<p class="modal-note">No strategy-family memory yet.</p>`}
  ...
  if (action === "summary") {
    ...
    const insufficient = (payload.branchesInsufficientEvidence || []).slice(0, 8).map(renderAutopilotBranch).join("");
    ...
    return `
      ...
      ${learning ? `<h4 class="modal-section-title">What Autopilot Learned</h4><ul class="backtest-warnings">${learning}</ul>` : ""}
      <div class="metric-grid">
        <div class="metric"><span>Branches tested</span><strong>${payload.branchesTested ?? 0}</strong></div>
        <div class="metric"><span>Real trading</span><strong>${payload.safety?.realTradingEnabled ? "enabled" : "disabled"}</strong></div>
        <div class="metric"><span>Research only</span><strong>${payload.safety?.researchOnly ? "yes" : "no"}</strong></div>
      </div>
      <h4 class="modal-section-title">Next Jobs</h4>
      ${jobs || `<p class="modal-note">No queued recommended jobs.</p>`}
      ...
      <h4 class="modal-section-title">Insufficient Evidence</h4>
      <table class="trade-table"><thead><tr><th>Category</th><th>Strategy</th><th>Market</th><th>Period</th><th>Trades</th><th>PF</th><th>Return</th><th>Neg Folds</th><th>Stress</th></tr></thead><tbody>${insufficient || `<tr><td colspan="9">No insufficient-evidence branches in memory.</td></tr>`}</tbody></table>
  ...
  const families = (memory.strategyFamilies || payload.strategyFamilies || []).slice(0, 8).map(renderAutopilotFamily).join("");
...
async function loadResearchPaperCandidates() {
...
function renderResearchPaperCandidates(payload) {
...
async function loadResearchMultiStrategyOptimizerBatch() {
  const host = document.querySelector("#research-multi-strategy-optimizer-batch-panel");
  if (!host) return;
  try {
    host.innerHTML = `<p class="pane-status">Running bounded optimizer batch...</p>`;
    const payload = await apiGet("/api/research/multi-strategy-optimizer-batch");
    host.innerHTML = renderResearchMultiStrategyOptimizerBatch(payload);
  } catch (error) {
    host.innerHTML = `<p class="pane-status">Multi-strategy optimizer batch could not load: ${escapeHtml(error.message)}</p>`;
  }
}

function renderResearchMultiStrategyOptimizerBatch(payload) {
  const search = payload.search || {};
  const summary = payload.summary || {};
  const rec = summary.recommendation || {};
  const best = summary.bestRawCandidate || summary.bestOverall || {};
  ...
  return `
    ...
    <div class="metric-grid">
      ...
      <div class="metric"><span>Rows</span><strong>${payload.rows?.length || 0}</strong></div>
      <div class="metric"><span>Evaluated combos</span><strong>${search.evaluatedCombos ?? "-"}</strong></div>
      <div class="metric"><span>Grid cap</span><strong>${search.maxCombosPerStrategy ?? "-"}</strong></div>
      <div class="metric"><span>Active</span><strong>${active.strategy ? `${escapeHtml(active.strategy)} ${escapeHtml(active.symbol || "")} ${escapeHtml(active.timeframe || "")}` : "-"}</strong></div>
      <div class="metric"><span>Saved path</span><strong>${escapeHtml(payload.savedPath || "-")}</strong></div>
    </div>
